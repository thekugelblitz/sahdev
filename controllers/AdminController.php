            ...
            <form method="post" action="<?php echo $actionUrl; ?>">
                ...
                <div class="panel panel-default" style="margin-bottom: 25px; border-left: 4px solid #0dcaf0;">
                    ...
                    <div class="panel-body">
                        ...
                        <p class="text-muted" style="margin-top: 8px; margin-bottom: 0; font-size:13px;">
                            When enabled, each successful cron or ticket-list “analyze” run pushes tags into WHMCS so they show in the <strong>native Tag Cloud</strong> field on the ticket (sidebar). Tags use the <code>ai-</code> prefix (e.g. <code>ai-billing</code>) so they stay distinct from manual tags; Sahdev replaces previous <code>ai-*</code> entries on that ticket when it re-analyzes. Sahdev tries the internal <code>UpdateTicket</code> API, the <code>tbltickets.tags</code> column if your build has it, and the standard tag tables (typically <code>tbltags</code> plus either <code>tbltickettags</code> or <code>tbltaglinks</code>). After enabling, run analysis again (or wait for cron) so existing tickets get tags. The ticket list still shows tag chips in Sahdev insights.
                        </p>
                    </div>
                </div>
